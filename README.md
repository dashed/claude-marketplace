...
| **comment-slop** | Find and remove AI-slop comments — ones that restate the code, describe another layer's behavior, or narrate planning that never shipped — while protecting the comments that carry real reasoning. Use when a reviewer calls a comment useless, unclear, or AI-written, when auditing comments and docstrings on a branch or PR, or before sending a change for review. Encodes a three-clause test (true at this layer, not already visible in the code, not stated better nearby), a seven-mode taxonomy with real examples and reviewer reactions, a "what to keep" table with the asymmetry that a deleted rationale costs more than a bland comment, and a diff-scoped keep/rewrite/delete workflow. Includes why mechanically shortening comments makes slop worse. | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/comment-slop) |
| **jj-hunk** | Programmatic hunk selection for the jj (Jujutsu) VCS. Select individual diff hunks with a query language instead of an interactive TUI, then split, commit, squash, diffedit, restore, or absorb them. Use when splitting a jj commit, committing or squashing part of the working copy, undoing single hunks, or selecting changes by file/content/function/class. Documents the traps an agent hits silently: `restore` reads its ids from a reversed listing, `absorb` exits 0 when it refuses, and content-level predicates never reach binaries, symlinks, mode flips, pure renames or empty adds. Tool: [jj-hunk](https://github.com/dashed/jj-hunk) | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/jj-hunk) |
| **ruff** | Astral's one fast (Rust) binary that is both a Python linter (`ruff check`) and a Black-compatible formatter (`ruff format`). Use when linting/formatting Python, configuring `[tool.ruff]`/`ruff.toml`, choosing rules with `select`/`extend-select`/`ignore`, looking up codes (F401/E501/B008), applying fixes (`--fix`, `--unsafe-fixes`), or suppressing (`# noqa`, `# ruff: ignore[...]`). Verified against **ruff 0.16.3**, incl. the 0.16 default-rule-set expansion (59 → 413) and preview gating. Tool: [ruff](https://github.com/astral-sh/ruff) | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/ruff) |
| **ty** | Astral's extremely fast Python type checker and language server (**beta**). Use when running/configuring `ty check`, reading diagnostics, setting rule levels in `[tool.ty]`/`ty.toml`, writing `# ty: ignore[rule]` suppressions, debugging environment/module resolution, wiring ty into CI, or migrating from mypy/pyright. Pinned to **ty 0.0.72**. Tool: [ty](https://github.com/astral-sh/ty) | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/ty) |
| **oxlint** | The extremely fast Rust JavaScript/TypeScript linter from the **oxc** project — an ESLint alternative needing no config to start. Use when linting JS/TS/JSX/Vue, writing `.oxlintrc.json`, selecting rules via `plugins` + `categories`, running `--fix`/`--fix-suggestions`/`--fix-dangerously`, or migrating from ESLint. Verified against **oxlint 1.79.0**. Tool: [oxlint](https://github.com/oxc-project/oxc) | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/oxlint) |
| **oxfmt** | The fast Rust formatter from the **oxc** project — a Prettier-compatible alternative for JS/TS/JSX plus JSON, CSS, YAML, TOML, GraphQL, Markdown and Vue. Use when formatting or checking with oxfmt, configuring `.oxfmtrc.json`, or migrating off Prettier/Biome. Pinned to **oxfmt 0.64.0**; note bare `oxfmt` rewrites files in place. Tool: [oxfmt](https://github.com/oxc-project/oxc) | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/oxfmt) |
| **python-complexity** | Measure per-function cyclomatic (`ruff check --select C901`) and cognitive (`complexipy`) complexity in Python with no project install, then count what they miss between functions: hops, pass-through layers, selector parameters. Use when asking which functions to refactor first, whether a function or call path is too complex or over-layered, when every function scores low but the code is hard to follow, when triaging a legacy codebase, or when verifying a refactor reduced rather than relocated complexity. | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/python-complexity) |
| **test-sloc-cut** | Shrink a test suite without losing an asserted fact or a covered branch: a fact matrix of what each test pins plus per-test branch coverage and a verified covering set, deleting only on the intersection, with a mutant check for every relocated fact. Use when asked to trim, dedupe, consolidate, or reduce tests or test SLOC, when a suite grew by accretion, when deciding whether to delete, fold, or parametrize tests, or before cutting a test on 'coverage is unchanged' alone. Python/pytest. | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/test-sloc-cut) |

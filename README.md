...
| **handoff** | Compact the current conversation into a handoff document for another agent to pick up. Use when ending a session with unfinished work, transferring context to a fresh agent or a new session, or approaching context limits. Saves to the OS temp dir (or the workspace root with `--workspace`), suggests skills for the next agent, references existing artifacts instead of duplicating them, and redacts secrets. | [mattpocock](https://github.com/mattpocock/skills/tree/main/skills/productivity/handoff) |

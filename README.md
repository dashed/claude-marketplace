...
| **python-complexity** | Measure per-function cyclomatic (`ruff check --select C901`) and cognitive (`complexipy`) complexity in Python to find refactor targets, with no project install. Use when asking which functions to refactor first, whether a function is too complex, when triaging a legacy codebase, or when verifying a refactor actually reduced complexity. | [dashed](https://github.com/dashed/claude-marketplace/tree/master/plugins/python-complexity) |

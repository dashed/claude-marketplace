...
async function checkLinearApiKey(): Promise<{
  valid: boolean;
  issues: string[];
  suggestions: string[];
}> {
...
async function checkSdkInstalled(): Promise<{
  installed: boolean;
  issues: string[];
  suggestions: string[];
}> {
...
async function runSetupCheck(): Promise<SetupResult> {
  ...
  if (sdkResult.installed) {
    ...
    if (process.env.LINEAR_API_KEY) {
      ...
      allSuggestions.push('After installing SDK, set your API key (see instructions above)', '');
    }
  }

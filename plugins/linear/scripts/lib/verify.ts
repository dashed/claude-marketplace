...
export async function verifyProjectsForInitiative(
  ...
  for (const proj of projects.nodes) {
    ...
    if (verification.overall.passed) {
      ...
      summary.failed++;
      summary.issues.push(`${proj.name}: ${verification.overall.issues.join('; ')}`);
    }
  }
...
export function printVerificationReport(verification: ProjectVerification): void {
  ...
  console.log(
    `  Has Description: ${verification.project.hasDescription ? '✓' : '✗'} (${verification.project.descriptionLength} chars)`,
  );
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  async function main() {
    ...
    if (command === 'all') {
      ...
      console.log('Usage:');
      console.log(
        '  verify.ts all <initiativeId> [projectFilter]       - Verify all projects for initiative',
      );

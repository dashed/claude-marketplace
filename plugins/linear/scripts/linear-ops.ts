...
const commands: Record<string, (...args: string[]) => Promise<void>> = {
  async 'create-issue'(
    ...
    if (!projectName || !title) {
      console.error(
        'Usage: create-issue <project-name> <title> [description] [--priority 1-4] [--labels label1,label2]',
      );
  ...
  async 'create-project'(name: string, initiativeName?: string) {
    ...
    if (initiativeName) {
      ...
      if (initiatives.nodes.length === 0) {
        console.error(
          `[WARNING] Initiative "${initiativeName}" not found, creating unlinked project`,
        );
      } else {
  ...
  async 'create-project-update'(projectName: string, body: string, healthFlag?: string) {
    if (!projectName || !body) {
      console.error(
        'Usage: create-project-update <project-name> <body> [--health onTrack|atRisk|offTrack]',
      );
      console.error(
        'Example: create-project-update "My Project" "## Summary\\n\\nWork completed..."',
      );
      process.exit(1);
    }
  ...
  async 'create-initiative-update'(initiativeName: string, body: string, healthFlag?: string) {
    if (!initiativeName || !body) {
      console.error(
        'Usage: create-initiative-update <initiative-name> <body> [--health onTrack|atRisk|offTrack]',
      );
      console.error(
        'Example: create-initiative-update "My Initiative" "## Phase Complete\\n\\n..."',
      );
      process.exit(1);
    }
  ...
  async 'create-sub-issue'(
    ...
    if (!parentIssue || !title) {
      console.error(
        'Usage: create-sub-issue <parent-issue> <title> [description] [--priority 1-4] [--labels label1,label2]',
      );
  ...
  async 'labels-set'(issueIdentifier: string, labelString: string, ...flags: string[]) {
    ...
    if (replaceMode) {
      ...
      const newIds = resolvedLabels.filter((l) => !existingIds.includes(l.id)).map((l) => l.id);

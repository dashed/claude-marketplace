...
- comment-slop 1.5.0: optional Jev judgments for whole comments and partial docstring reductions, using the general configurable helper; preserves documentation/runtime/tooling consumers and skips without helper or credentials. Frozen semantic evals and offline uv/Ruff/ty tests retain failures.
- `jev` skill: general-purpose typed consultations for plans, evidence checks, priorities, and counterchecks; standalone provider-configurable helper with optional credentials, request preview, and offline uv/Ruff/ty tests.
...
- python-complexity skill: the call-path census promoted to co-lead. SKILL.md opens with a "two censuses" table (per function / per call path — question, runs, signal, golf caught), "When to Use" splits into the two questions, the path census gets a runnable three-command quick start with expected numbers, and "Which layers stay" (the structure-or-safety test, the five structural costs as a table, the coincident-constants rule) moves up from the judgment reference. First marketplace use of the `when_to_use` frontmatter field, which the loader appends to the description under a combined 1,536-character cap — the pattern for any skill whose description is at the 1,024 cap and needs more triggers (python-complexity v1.2.0)
- python-complexity skill: measured basis for the change — five fresh-agent probes with layering-worded prompts all invoked the skill and reached the path census, so the material worked when found; the emphasis fixes target the surfaces a reader meets first (first layering concept at character 188 of the description, path section at 55 % of SKILL.md with no inline command, Codex short description truncating at "hops..."). Third-codebase run on the `coverage` source added to the reference: static hop counts on object-heavy code are floors and must be reported with their `unresolved`/`heuristic` buckets (python-complexity v1.2.0)

### Fixed
- python-complexity skill: `scripts/hops_dyn.py` crashed on any package containing a module named like a stdlib one (`html.py`, `types.py`, `parser.py`) because it put the package directory first on `sys.path`; packages are now imported through their parent (python-complexity v1.2.0)
...
- python-complexity skill: ruff `PLR1702` / `PLR0913` / `PLR0915` as the rules that see the skill's own blind-spot fixtures in the same run as C901 (a 5-deep `with` or `try/finally` pyramid: complexipy 0, PLR1702 5), with their measured quirks and the preview-rule silent failure (stderr warning, `All checks passed!`, exit 0); measured silent failures of `python -m trace --trackcalls`, `code2flow` (`@overload` truncation to 6 nodes at exit 0, same-basename merging), `lizard -ENS` / `-Eduplicate`, `radon mi` / `hal`, and radon's shared nested-class blind spot (python-complexity v1.1.0)

### Changed
- python-complexity skill: workflow steps 5–6 count what a split adds and re-read the path; "Never gate CI" names splitting as the third golf; thresholds sourced first-hand from McCabe 1976 (the paper prescribes modularizing above 10 and proves in §IV that it does not reduce the total); cyclomatic-ruff.md's "do not aggregate" qualified; the description gains between-function triggers (over-layered, too indirect, every function scores low, comparing against the path it replaces, reduced rather than relocated) (python-complexity v1.1.0)
